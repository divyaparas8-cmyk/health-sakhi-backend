...
async function testUploadFrontendSimulation() {
  ...
  formData.append('mediaFile', fs.createReadStream('dummy.mp4'));

  try {
    const response = await axios.post('http://localhost:5000/api/v1/admin/content-assets/upload', formData, {
      headers: {
        'Content-Type': 'multipart/form-data', // This is exactly what frontend does!
